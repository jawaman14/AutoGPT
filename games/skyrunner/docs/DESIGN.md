...
```
 Pilot's game (3D, Panda3D) ─────────────┐   authoritative Session (JSBSim + AI + rules), 60 Hz
   └─ listen server (asyncio thread) ────┤   snapshots 15 Hz, filtered per role (fog of war)
                                         │
...
- **Phase 3:** a remote *pilot* (interceptor or second runner) needs UDP with client-side
  prediction of their own JSBSim instance (ENet, or Panda3D's distributed networking), with the
...
- **Dedicated server:** `Session` has no rendering dependency, so a headless server is the same
  code minus the Panda3D window (`python -m skyrunner.net.server`).
