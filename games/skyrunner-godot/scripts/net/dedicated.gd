...
func _init() -> void:
	...
	sess = Session.new({"mode": Roles.POLICE, "seed": seed, "map_seed": MapCity.SEED, "ground_war": true, "chronicle": true, "agency": true, "family": true, "island": true})  # AI runs the desk until a controller joins
	srv = HostServer.new()
	srv.attach(sess)
