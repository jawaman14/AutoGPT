...
#   tools/record_demo.sh out.mp4 <zone> <law> <tactic> <seed> [graphics] [max_s] [hour] [speedup] [sky] [moon]
set -euo pipefail
cd "$(dirname "$0")/.."
OUT=$1; shift
SPEED=${9:-2}
...
xvfb-run -a -s "-screen 0 1280x720x24" "$GODOT" --rendering-method gl_compatibility --audio-driver Dummy \
  --resolution 1280x720 --write-movie "$TMP/demo.avi" --fixed-fps 30 --script res://tools/demo.gd -- "${@:1:7}" "${@:9}" 2>&1 \
